...
def main_app(config):
    ...
    frame = tk.Frame(root)
    frame.pack(pady=20)
    #place to enter master password
    master_password = tk.Entry(frame, width=30)
    master_password.grid(row=0, column=0, padx=(0, 10)) 
    #place to enter password to encrypt
    plaintext = tk.Entry(frame, width=30)
    plaintext.grid(row=1, column=0, padx=(0, 10))
    #place to put common name
    common_name = tk.Entry(frame, width=30)
    common_name.grid(row=2, column=0, padx=(0, 10))
    #need dropdown to choose a common name from for decryption
    ...
    def clear_field():
        #clear all user-input fields - i think
    ...
    generate_password.pack(pady=10)
    ...
    root.after(10000, clear_field) #clear all user-input fields, actually. 
    root.mainloop()
...
#3. build out function to generate a new password and save it with a common name
#4.
#5.